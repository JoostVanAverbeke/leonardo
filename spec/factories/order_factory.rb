FactoryBot.define do
  factory :order do
    ...
    filler_order_number { "#{Faker::Date.between(from: 5.days.ago, to: Date.today).strftime('%Y%m%d')}-#{Faker::Number.unique.number(digits: 8)}" }
    ...
    ordering_provider_id { association :hc_provider }
